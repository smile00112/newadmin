<x-admin::layouts>
        ...
        <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">Маркетинговые кампании</p>
                </div>
            </div>
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-campaigns-template">
            <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
                <div class="flex items-center gap-3">
                    <div class="flex items-center justify-center w-11 h-11 rounded-xl" style="background: linear-gradient(135deg, #ec4899 0%, #db2777 100%); box-shadow: 0 4px 15px rgba(236,72,153,0.3); min-width:44px;">
                        <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-x-2.5">
                    @if (bouncer()->hasPermission('marketing.communications.campaigns.create'))
                        <div class="primary-button" @click="openDrawer('create')">
                            @lang('admin::app.marketing.communications.campaigns.index.create-btn')
                        </div>
                    @endif
                </div>
            ...
            <x-admin::datagrid
                :src="route('admin.marketing.communications.campaigns.index')"
                ref="datagrid"
            >
                <template #body="{
                    isLoading,
                    ...
                    <template v-else>
                        ...
                        >
                            ...
                            <p>@{{ record.name }}</p>
                            <p>@{{ record.subject }}</p>
                            <p>@{{ record.status ? 'Активна' : 'Неактивна' }}</p>
            ...
            app.component('v-campaigns', {
                ...
                methods: {
                    openDrawer(mode, record = null) {
                        ...
                        if (mode === 'create') {
                            targetUrl = window.location.origin + '/admin/marketing/communications/campaigns/create-panel';
                            targetId = 'create';
                        } else {
                            targetUrl = window.location.origin + '/admin/marketing/communications/campaigns/edit-panel/' + record.id;
                            targetId = record.id;
                        }
                    ...
                    preloadRecord(record) {
                        ...
                        this.hoverTimer = setTimeout(() => {
                            ...
                            this.isDrawerLoading = true;
                            this.iframeSrc = window.location.origin + '/admin/marketing/communications/campaigns/edit-panel/' + record.id;
                        }, 150);
                    },
