...
class SendLiveActivityPushJob implements ShouldQueue
{
    ...
    public function handle(ApnsLiveActivityService $apnsService): void
    {
        ...
        if (! $order) {
            Log::warning('SendLiveActivityPushJob: order not found', ['order_id' => $this->orderId]);
        ...
        if (! $tokenRecord) {
            Log::debug('SendLiveActivityPushJob: token record not found (likely already deleted)', [
        ...
        $apnsService->send($tokenRecord, $order);
    }
}
