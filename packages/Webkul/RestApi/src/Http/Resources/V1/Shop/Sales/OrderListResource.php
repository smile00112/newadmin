...
class OrderListResource extends JsonResource
{
    ...
    {
        try {
            $p = $this->payment->method_title ?? null;
        } catch (\Throwable $th) {
            $p = null;
        }
        ...
        return [
            ...
            'bonus_amount_accrued'                    => $this->bonus_amount_accrued ?? 0,
            'formatted_bonus_amount_accrued'          => core()->formatPrice($this->bonus_amount_accrued ?? 0, $this->order_currency_code),
            'base_bonus_amount_accrued'               => $this->base_bonus_amount_accrued ?? 0,
            'formatted_base_bonus_amount_accrued'     => core()->formatBasePrice($this->base_bonus_amount_accrued ?? 0),
