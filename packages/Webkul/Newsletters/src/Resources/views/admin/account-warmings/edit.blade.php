<x-admin::layouts>
    <x-slot:title>
        {{ __('newsletters::app.admin.account-warmings.title') }} - {{ __('newsletters::app.common.actions.edit') }}
    </x-slot:title>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ __('newsletters::app.common.actions.edit') }} {{ __('newsletters::app.admin.account-warmings.title') }}
            </h2>
        </div>

        <form action="{{ route('admin.newsletters.account-warmings.update', $warming->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
                <!-- Name -->
                <div>
                ...
                <div>
                    ...
                    >
                        @foreach($whatsappInstances as $instance)
                            <option value="{{ $instance->id }}" {{ in_array($instance->id, old('whatsapp_instance_ids', $warming->selected_account_ids ?? [])) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div id="phrases-container" class="space-y-4">
                        @foreach(old('phrases', $warming->phrases ?? []) as $index => $phrase)
                            <div class="phrase-pair border border-gray-300 dark:border-gray-600 rounded-md p-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <textarea
                                            name="phrases[{{ $index }}][question]"
                                            rows="3"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                            required
                                        >{{ old("phrases.$index.question", $phrase['question'] ?? '') }}</textarea>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        ...
                                        <textarea
                                            name="phrases[{{ $index }}][answer]"
                                            rows="3"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                            required
                                        >{{ old("phrases.$index.answer", $phrase['answer'] ?? '') }}</textarea>
                                    </div>
                                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input
                            ...
                            name="delay_from"
                            id="delay_from"
                            value="{{ old('delay_from', $warming->delay_from ?? 5) }}"
                            min="1"
                            max="3600"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                        >
                    ...
                    <div>
                        ...
                        <input
                            ...
                            name="delay_to"
                            id="delay_to"
                            value="{{ old('delay_to', $warming->delay_to ?? 5) }}"
                            min="1"
                            max="3600"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                        >
                ...
                <div>
                    ...
                    <input
                        ...
                        name="start_at"
                        id="start_at"
                        value="{{ old('start_at', $warming->start_at ? $warming->start_at->format('Y-m-d\TH:i') : '') }}"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                    >
                    @error('start_at')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
    ...
    <script>
        let phraseIndex = {{ count(old('phrases', $warming->phrases ?? [])) }};
