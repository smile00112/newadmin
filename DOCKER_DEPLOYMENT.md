...
### Права на скрипты (update.sh, deploy.sh)

После `git pull` или копирования кода на сервер права на скрипты могут сброситься до 644. Варианты:

- **Запуск через bash** (права восстановятся автоматически): `bash update.sh` или `bash deploy.sh` — скрипты сами выставят себе и соседу 755, в следующий раз можно вызывать `./update.sh` и `./deploy.sh`.
- **Вручную один раз:** `chmod +x update.sh deploy.sh`, затем `./update.sh`.

В репозитории для скриптов зафиксирован исполняемый бит (`git update-index --chmod=+x`). После `git pull` на Linux права 755 должны восстанавливаться автоматически; если этого не произошло (например, при копировании не через Git), используйте вариант выше.
...
```bash
chmod +x update.sh   # только если ещё не сделано (или запустите: bash update.sh)
./update.sh
```
