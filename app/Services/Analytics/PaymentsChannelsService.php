...
/**
 * H. Payments & Stability + I. Channels & Format
 *
 * All metrics derived from orders / order_payment / customers tables.
 */
class PaymentsChannelsService extends BaseAnalyticsService
{
    ...
    /**
     * Payment success rate by method — derived from order_payment + order status.
     * A payment is "successful" if the order status is not cancelled/failed/fraud.
     */
    public function getPaymentSuccessRate(): array
    {
        ...
        $byMethod = $query
            ...
            ->selectRaw("
                COUNT(*) as total,
                SUM(CASE WHEN orders.status NOT IN ('canceled','cancelled','failed','fraud') THEN 1 ELSE 0 END) as successes,
                SUM(CASE WHEN orders.status IN ('canceled','cancelled','failed','fraud') THEN 1 ELSE 0 END) as failures
            ")
    ...
    /**
     * Payment fail reasons: cancelled orders grouped by payment method.
     */
    public function getPaymentFailReasons(int $limit = 10): array
    {
        return DB::table('order_payment')
            ->join('orders', 'orders.id', '=', 'order_payment.order_id')
            ->whereBetween('orders.created_at', [$this->startDate, $this->endDate])
            ->whereIn('orders.status', ['canceled', 'cancelled', 'failed', 'fraud'])
    ...
    /**
     * Crash-free sessions estimate: orders without failures / total orders.
     */
    ...
    /**
     * Average screen latency: order processing time as proxy.
     */
    public function getAvgScreenLatency(): array
    {
        ...
        return [
            'avg_ms'      => round(((float) ($avg ?? 0)) * 1000, 0),
            'sample_size' => DB::table('orders')->whereBetween('created_at', [$this->startDate, $this->endDate])->count(),
        ];
    }
    ...
    /**
     * Dine-in vs Take-away based on table_number: has table = dine_in, no table = takeaway.
     */
    public function getDineInVsTakeaway(): array
    {
        ...
        $dineIn = (clone $query)->whereNotNull('table_number')->count();
    ...
    /**
     * Orders by location: group by channel_name as location proxy.
     */
    ...
    /**
     * Post-order rating from orders.rating field.
     */
    ...
    /**
     * NPS from orders.rating: 5 = promoter, 4 = passive, 1-3 = detractor.
     * (Using 1-5 scale mapped to NPS logic.)
     */
    public function getNPS(): array
    {
        ...
        // 5-star scale: 5 = promoter, 4 = passive, 1-3 = detractor
    ...
    /**
     * Complaints stats: canceled orders as complaints proxy.
     */
    public function getComplaintsStats(): array
    {
        ...
        // Top cancel reasons by payment method
    ...
    /**
     * Kiosk uptime: all orders as "uptime" proxy — shows active order channels.
     */
