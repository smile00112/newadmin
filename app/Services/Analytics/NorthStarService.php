...
/**
 * North Star + Financial Metrics
 *
 * Formulas:
 *   Online Order Share    = online_orders / total_orders * 100
 *   AOV                   = GMV / total_orders
 *   GMV                   = SUM(orders.base_grand_total) WHERE status NOT IN (cancelled, failed, fraud)
 *   Avg Time Order→Ready  = AVG(total_seconds) from analytics_order_timestamps
 *   Orders within SLA     = COUNT(within_sla = true) / total_orders * 100
 *   Repeat Rate           = customers_with_2plus_orders / customers_with_any_order * 100
 */
class NorthStarService extends BaseAnalyticsService
{
    /**
     * All North Star KPIs in one call.
     */
    ...
    public function getOnlineOrderShare(): array
    {
        ...
        $online = (clone $query)->whereIn('channel_name', ['app', 'kiosk'])->count();
